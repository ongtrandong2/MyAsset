...
export default function Income() {
    ...
    let Income = IncomeOutcome.filter((item) => {
        return (
            item.isIncome === true && moment(item.time).format('MM') === itemSelected
        )
    })
    ...
    let Income_ByYear = IncomeOutcome.filter((item) => {
        return (
            item.isIncome === true && moment(item.time).format("YYYY") === yearSelected.toString()
        )
    })
    ...
    if (acceptDateStart !== undefined && acceptDateEnd !== undefined) {
        ...
        let Income_ByOption = IncomeOutcome.filter((item) => {
            let d = new Date(moment(item.time).format('YYYY-MM-DD'));
            return (
                item.isIncome === true && d.getTime() >= acceptDateStart.getTime() && d.getTime() <= acceptDateEnd.getTime()
            )
        })
        //console.log(Outcome_ByOption);
        Income_ByOption.map((item) => {
